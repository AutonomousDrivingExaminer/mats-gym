...
class SensorCallBack(ABC, Generic[T]):
    def __init__(
        self, tag: str, type: str, sensor: Sensor, sensor_interface: SensorInterface
    ) -> None:
        self._tag = tag
...
class GnssCallback(SensorCallBack[carla.GnssMeasurement]):
    def __call__(self, data: carla.GnssMeasurement) -> None:
        array = np.array(
            [data.latitude, data.longitude, data.altitude], dtype=np.float64
        )
        self.update(array, data.frame)
        ...
        self.update(
            {
                ...
                "impulse": np.array(
                    [
                        data.normal_impulse.x,
                        data.normal_impulse.y,
                        data.normal_impulse.z,
                    ],
                    dtype=np.float32,
                ),
                "other_location": np.array(
            ...
            crossings.append(
                {
                    ...
                    "location": np.array(
                        [
                            crossing.transform.location.x,
                            crossing.transform.location.y,
                            crossing.transform.location.z,
                        ],
                        dtype=np.float32,
                    ),
                }
        ...
        self.update(
            {
                ...
                "obstacle_location": np.array(
                    [obstacle_location.x, obstacle_location.y, obstacle_location.z],
                    dtype=np.float32,
                ),
