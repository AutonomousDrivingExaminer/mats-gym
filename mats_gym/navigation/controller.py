...
class VehiclePIDController:
    ...
    def __init__(
            ...
            args_lateral: dict[str, Any],
            args_longitudinal: dict[str, Any],
            offset=0,
            max_throttle=0.75,
            max_brake=0.3,
            max_steering=0.8):
    ...
    def change_longitudinal_PID(self, args_longitudinal: dict[str, Any]) -> None:
        """Changes the parameters of the PIDLongitudinalController"""
        self._lon_controller.change_parameters(**args_longitudinal)

    def change_lateral_PID(self, args_lateral: dict[str, Any]) -> None:
        """Changes the parameters of the PIDLongitudinalController"""
...
class PIDLongitudinalController:
    ...
    def run_step(self, target_speed: float, debug=False) -> float:
        """
    ...
    def _pid_control(self, target_speed: float, current_speed: float) -> float:
        ...
        return np.clip((self._k_p * error) + (self._k_d * _de) + (self._k_i * _ie), -1.0, 1.0).item()
...
class PIDLateralController():
    ...
    def _pid_control(self, waypoint: carla.Waypoint, vehicle_transform: carla.Transform) -> float:
        ...
        return np.clip((self._k_p * _dot) + (self._k_d * _de) + (self._k_i * _ie), -1.0, 1.0).item()
