...
class CameraRenderer:
    ...
    def __init__(self,
                 ...
                 camera_transform: carla.Transform = carla.Transform(carla.Location(x=-5.5, z=2.8), carla.Rotation(pitch=-15)),
                 width: int = 1280,
                 height: int = 720,
    ...
    def _spawn_camera(self, camera_type: str, width: int, height: int):
        ...
        camera: carla.Sensor = self._world.spawn_actor(camera, self._camera_transform, attach_to=self._actor)
        camera.listen(lambda image: CameraRenderer._parse_image(weak_self=weakref.ref(self), image=image))
        return camera
    ...
    def reset(self, client: carla.Client = None):
        ...
        self._camera = self._spawn_camera(camera_type=self._camera_type, width=self._width, height=self._height)
        self._frames = []
    ...
    def _add_text(self, text: str, image: np.ndarray):
        ...
        (width, height), _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_PLAIN, scale, thickness)
